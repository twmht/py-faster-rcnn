...
class common(imdb):
    def __init__(self, image_set):
        ...
        self._classes = map(str, range(0, 10)) + [u for u in string.lowercase]
        self._classes.insert(0, '__background__')
        self._classes.insert(1, 'check')
        self._classes = tuple(self._classes)

        assert len(self._classes) == 38
    ...
    @property
    def num_images(self):
        if self._num_images == None:
            self._num_images = 0
            for value in self._image_index.values():
                self._num_images = self._num_images + len(value)
    ...
    def _setup(self):
        self._image_loc = os.path.join(self._data_path, 'Image')
        self._index_loc = os.path.join(self._data_path, 'Index')
        self._annotation_loc = os.path.join(self._data_path, 'Annotation')

        assert os.path.exists(self._image_loc), \
                'Path does not exist: {}'.format(self._image_loc)
        assert os.path.exists(self._index_loc), \
                'Path does not exist: {}'.format(self._index_loc)
    ...
    def _load_image_set_index(self):
        ...
        image_index = OrderedDict()
        for db_name in self._db_names:
            image_index_loc = os.path.join(self._index_loc, db_name + '.txt')
            with open(image_index_loc) as f:
                image_index[db_name] = [x.strip() for x in f.readlines()]
        return image_index
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, self._image_set)
    ...
    def gt_roidb(self):
        ...
        gt_roidb = []
        for db_name, indexes in self._image_index.iteritems():
            db_path = os.path.join(self._data_path, 'Annotation', db_name)
    ...
    def _load_pascal_annotation(self, value):
        """
